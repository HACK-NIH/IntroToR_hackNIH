---
title: "Data Simulation for Dataset (Azmi)"
author: "Mohd Azmi"
date: last-modified
...
-   This document is a data simulation for dataset (Azmi) using R programming language.
-   The purpose of this document is to simulate a dataset for a our training program
-   As for today (2024-03-05), this dataset is only placeholder and will be updated accordingly
-   Anyone can use this dataset.
-   If we want to standardised this dataset, we need to discuss further. Im open for any suggestion.

## Library

-   pacman
    -   load multiple package at once
    -   If the package is not yet install, it will install automatic
-   tidyverse & stringi
    -   this should be obvious
-   labelled & haven
    -   for labelled data (SPSS)
<!--# even if you want to stay away from SPSS, majority of NIH user still depend on SPSS -->
-   simstudy
    -   for data simulation
<!--# just some basic simulation. require further reading -->

```{r}
#| label: setup

pacman::p_load(tidyverse, stringi, labelled, haven, simstudy)
```
...
-   This dataset simulation was copied from my other training program.
-   Not commented yet because initially it for my own use.
-   But since it may be use in our project, I will comment it later.


### Dataset with Labels

-   This is dataset with labels
-   Suitable for csv, xlsx or any format
-   Note: if export to SPSS, the index level with start with 1
...
# write_csv(chol_labds, "../data/chol_labds.csv")
```

### Dataset with Numeric

-   This is raw dataset, i.e.:
    -   Everything is numeric
    -   For categorical data, the level start with value 0
    -   No labels
    -   No factor
-   Suitable for data wrangling exercise
-   There is also data dictionary
...
# write_csv(chol_rawds, "../data/chol_rawds.csv")
...
# write_csv(chol_raw_dict, "../data/chol_raw_dict.csv")
...
-   This is dataset with SPSS labels
-   Can be exported to SPSS format
...
# write_sav(chol_spssds, "../data/chol_spssds.sav")
```
...
-   There is mixture of multiple condition.
    -   I'll put some context later
-   Not commented yet because initially it for my own use.
...
```{r}
# write_sav(asthma_ds, "../data/asthma_ds.sav")
# write_sav(asthma24_long, "../data/asthma24_long.sav")
# write_sav(asthma24_wide, "../data/asthma24_wide.sav")

# write_sav(asthma_pre, "../data/asthma_pre.sav")
# write_sav(asthma_post, "../data/asthma_post.sav")
```
...
```{r}
asthmads_sav <- read_sav("../data/asthma_ds.sav")

asthma_ds_colnamelabel <- map(asthmads_sav, possibly(summarytools::label, NA)) %>% 
...
#              val_lab = "Value Label") %>%
#   gtsave(., filename = "../data/asthma_data_dictionary.docx")
...
```{r}
# write.csv(asthma_ds, "../data/asthma_ds.csv")

# write.csv(zap_labels(asthmads_sav), "../data/asthma_ds_nolab.csv")
